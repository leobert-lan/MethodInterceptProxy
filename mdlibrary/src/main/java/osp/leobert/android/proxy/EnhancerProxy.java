...
import osp.leobert.android.proxy.dx.Code;
import osp.leobert.android.proxy.dx.Comparison;
import osp.leobert.android.proxy.dx.DexMaker;
import osp.leobert.android.proxy.dx.FieldId;
import osp.leobert.android.proxy.dx.Label;
import osp.leobert.android.proxy.dx.Local;
import osp.leobert.android.proxy.dx.MethodId;
import osp.leobert.android.proxy.dx.TypeId;
...
import osp.leobert.android.proxy.method.MethodProxyExecutor;
...
public class EnhancerProxy {
	...
	public EnhancerProxy(Context context) {
...
//		System.out.println("[EnhancerProxy::create()] Create class extends from \"" + superclass.getName() + "\" stored in " + cacheDir);
		...
		try {
			ClassLoader loader = dexMaker.generateAndLoad(EnhancerProxy.class.getClassLoader(), new File(cacheDir));
	...
	private <S> void generateFieldsAndMethods(DexMaker dexMaker, TypeId<?> superType, TypeId<S> subType) {
		...
		TypeId<MethodProxyExecutor> methodProxyExecutorType = TypeId.get(MethodProxyExecutor.class);
		...
		MethodId methodId = methodProxyExecutorType.getMethod(TypeId.OBJECT, "executeMethod", classType, stringType, classesType, objectsType, objectType);
...
//			}
			...
			methodId = methodProxyExecutorType.getMethod(TypeId.OBJECT, "executeInterceptor",methodInterceptorsType,callbackFilterType, methodInterceptorType, classType, stringType, classesType, objectsType, objectType);
